...
# Request Dynamic Runtime Permission on Android
if platform == 'android':
    from android.permissions import request_permissions, Permission
    request_permissions([Permission.RECORD_AUDIO, Permission.FOREGROUND_SERVICE])
...
class PhoneControllerApp(App):
    def build(self):
        ...
        self.chat_logs = Label(
            text="[System]: App Started with Dynamic Permissions.\n", 
            size_hint_y=None,
            halign='left',
            valign='top'
        )
        ...
        self.user_input = TextInput(hint_text="Type command...", multiline=False)
    ...
    def toggle_mic(self, instance):
        ...
        if self.mic_active:
            ...
            self.status_label.text = "[ Status: Mic Listening... ]"
            self.chat_logs.text += "\n[System]: Mic Active."
        else:
            self.mic_btn.text = "Turn Mic ON"
            self.mic_btn.background_color = (0.2, 0.8, 0.2, 1)
            self.status_label.text = "[ Status: Mic OFF ]"
            self.chat_logs.text += "\n[System]: Mic Stopped."
